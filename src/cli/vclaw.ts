...
async function handleVideoFilmmakingPrompts(args: string[]): Promise<void> {
  ...
  const phaseFlag = parseFlagValue(args, '--phase');
  if (phaseFlag !== undefined && !['storyboard', 'video'].includes(phaseFlag)) {
    throw new Error('video filmmaking-prompts --phase must be one of storyboard, video');
  }
  // Joey opt-in cinematography flags. All default off → output byte-identical
  // to today. `--realism` enables the capture-realism block (with optional
  // `--wet` and `--haze`); `--lighting`/`--grade` swap the rich-suffix registers;
  // `--background` appends a backdrop-plate clause; `--sheet` selects the
  // character-sheet layout.
  const sheetFlag = parseFlagValue(args, '--sheet');
  if (sheetFlag !== undefined && !['8-shot', '6-panel'].includes(sheetFlag)) {
    throw new Error('video filmmaking-prompts --sheet must be one of 8-shot, 6-panel');
  }
  const hazeFlag = parseFlagValue(args, '--haze');
  if (hazeFlag !== undefined && !['thin', 'light', 'heavy'].includes(hazeFlag)) {
    throw new Error('video filmmaking-prompts --haze must be one of thin, light, heavy');
  }
  const backgroundFlag = parseFlagValue(args, '--background');
  if (backgroundFlag !== undefined && !['mid-gray', 'white', 'black'].includes(backgroundFlag)) {
    throw new Error('video filmmaking-prompts --background must be one of mid-gray, white, black');
  }
  ...
  const result = await generateFilmmakingPrompts({
    ...
    ...(lightingFlag !== undefined ? { lightingId: lightingFlag } : {}),
    ...(gradeFlag !== undefined ? { gradeId: gradeFlag } : {}),
    ...(backgroundFlag !== undefined ? { plateKind: backgroundFlag as 'mid-gray' | 'white' | 'black' } : {}),
...
async function handleVideoMultiShot(args: string[]): Promise<void> {
  const basePreset = resolveMultiShotPreset(args);
  // Joey opt-in: --genre swaps the preset's style line via resolveStyleLine
  // (Nolan fallback for unknown/absent genres). Omitting --genre keeps the
  // resolved preset byte-identical to today.
  const genreFlag = parseFlagValue(args, '--genre');
  const preset = genreFlag !== undefined
    ? { ...basePreset, styleLine: resolveStyleLine(genreFlag) }
    : basePreset;
  ...
  // Optional rendered-format selection. The default keeps the original JSON
  // output unchanged (backward compatible). The other formats render the shot
  // plan via the dedicated composers, then wrap the text for bilingual delivery
  // (default `en` = one fenced block). Translation is offline/identity here —
  // `--lang zh|en+zh` only changes the wrapper structure, not the text content.
  const formatFlag = parseFlagValue(args, '--format') ?? 'default';
  if (!['default', 'seedance-paragraph', 'per-shot'].includes(formatFlag)) {
    throw new Error('video multi-shot: --format must be one of default, seedance-paragraph, per-shot');
  }
  const langFlag = parseFlagValue(args, '--lang') ?? 'en';
  if (!['en', 'zh', 'en+zh'].includes(langFlag)) {
    throw new Error('video multi-shot: --lang must be one of en, zh, en+zh');
  }
  // Optional opening-hook directive (named HOOK_PATTERNS) and spoken dialogue
  // (withDialogue). Both validate/parse eagerly so unknown/malformed values fail
  // fast regardless of format, then apply only on non-default formats as a
  // post-render text transform (the composers stay pure). Omitting either flag
  // leaves the rendered text byte-identical to today.
  const hookFlag = parseFlagValue(args, '--hook');
  if (hookFlag !== undefined && !HOOK_PATTERN_IDS.includes(hookFlag as HookPatternId)) {
    throw new Error(`video multi-shot: --hook must be one of ${HOOK_PATTERN_IDS.join(', ')}`);
  }
  const dialogueFlag = parseFlagValue(args, '--dialogue');
  let dialogue;
  if (dialogueFlag !== undefined) {
    try {
      dialogue = parseDialogueLine(dialogueFlag);
    } catch (err) {
      throw new Error(`video multi-shot: --dialogue ${(err as Error).message}`);
    }
  }
  if (formatFlag !== 'default') {
    const descriptor = resolveCategory(parseFlagValue(args, '--category'));
    let rendered = formatFlag === 'seedance-paragraph'
      ? composeSeedanceParagraph(plan, descriptor)
      : composePerShotFormat(plan, descriptor);
    if (dialogue) {
      const newlineIdx = rendered.indexOf('\n');
      const head = newlineIdx === -1 ? rendered : rendered.slice(0, newlineIdx);
      const tail = newlineIdx === -1 ? '' : rendered.slice(newlineIdx);
      rendered = `${withDialogue(head, dialogue)}${tail}`;
    }
    if (hookFlag !== undefined) {
      rendered = `Opening hook — ${resolveHookPattern(hookFlag as HookPatternId)}\n\n${rendered}`;
    }
    process.stdout.write(`${composeBilingual(rendered, langFlag as Lang)}\n`);
    return;
  }
