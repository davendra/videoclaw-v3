...
""",
    ...
    parser.add_argument(
        "--product",
        help="Project slug alias matching parallel_video_gen.py / generate_tts.py "
             "(e.g., 2026-05-27_wicc-7687375). Auto-resolved to projects/<slug>. "
             "Either --project or --product is required."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-bumper", action="store_true",
        help="Skip the 6s 'Match Day Analysis' opening bumper. By default, when "
             "projects/<slug>/images/bumper_frame.jpg AND assets/bunty/jingle.mp3 "
             "both exist, stitch_bunty prepends a 6s static-image+jingle bumper "
             "as the first segment of the final video. Generate bumper_frame.jpg "
             "by compositing Bunty over slide_001 via go-bananas edit_uploaded_image."
    )
...
def main():
    ...
    # Resolve project dir from either --project (full path) or --product (slug alias).
    if args.product:
        proj_arg = args.product
        # Accept a bare slug or an already-prefixed path; normalise to projects/<slug>.
        if not (os.path.sep in proj_arg or proj_arg.startswith("projects/")):
            proj_arg = os.path.join("projects", proj_arg)
    elif args.project:
        proj_arg = args.project
    else:
        print("ERROR: one of --project or --product is required", file=sys.stderr)
        sys.exit(2)
    ...
    # === Step 3.5: Optional opening bumper ===
    # When projects/<slug>/images/bumper_frame.jpg AND assets/bunty/jingle.mp3
    # both exist, build a 6s static-image-plus-jingle segment and prepend it as
    # the first segment of the final video. The bumper_frame.jpg is a per-match
    # composite of Bunty over slide_001 (the title card with team logos),
    # generated by the operator via go-bananas edit_uploaded_image so the team
    # crests are pixel-preserved rather than regenerated. The jingle is a
    # shared 6s Bunty "Match Day Analysis" sting at assets/bunty/jingle.mp3.
    # The bumper is re-encoded to match the existing segments' params
    # (1280x720, 24fps, h264 + aac 44.1kHz stereo) so demuxer concat -c copy
    # works without DTS warnings or AV drift.
    bumper_seg = None
    if not args.no_bumper:
        bumper_frame = os.path.join(project, "images", "bumper_frame.jpg")
        # The jingle lives in the repo's shared assets/ dir. Walk up from this
        # script's location (.claude/skills/video-replicator/scripts/) to the
        # repo root and resolve assets/bunty/jingle.mp3.
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
        jingle = os.path.join(repo_root, "assets", "bunty", "jingle.mp3")
        if os.path.exists(bumper_frame) and os.path.exists(jingle):
            print("\n=== Step 3.5: Building opening bumper ===")
            print(f"  Frame:  {bumper_frame}")
            print(f"  Jingle: {jingle}")
            bumper_seg = os.path.join(segments_dir, "seg_bumper.mp4")
            bumper_cmd = [
                "ffmpeg", "-y",
                "-loop", "1", "-framerate", "24", "-i", bumper_frame,
                "-i", jingle,
                "-t", "6",
            ...
            try:
                subprocess.run(bumper_cmd, check=True, capture_output=True, text=True)
                print(f"  Built: {bumper_seg}")
            except subprocess.CalledProcessError as e:
                print(f"  WARNING: bumper build failed ({e.returncode}); skipping bumper")
                print(f"    stderr tail: {e.stderr.strip()[-300:] if e.stderr else ''}")
                bumper_seg = None
        elif os.path.exists(bumper_frame) and not os.path.exists(jingle):
            print(f"  Bumper SKIPPED: {bumper_frame} exists but jingle not found at {jingle}")
        elif os.path.exists(jingle) and not os.path.exists(bumper_frame):
            print(f"  Bumper SKIPPED: {jingle} exists but bumper_frame.jpg not found at {bumper_frame} "
                  f"(generate via go-bananas edit_uploaded_image on slide_001)")
        # Silent skip when neither exists — bumper is opt-in via the two assets.

    # === Step 4: Concatenate ===
    print("\n=== Step 4: Concatenating all segments ===")
    all_segs = [s for s in [bumper_seg] + intro_segs + slide_segs + outro_segs if s and os.path.exists(s)]

    print(
        f"  Total segments: {len(all_segs)} "
        f"({'1 bumper + ' if bumper_seg else ''}{len(intro_segs)} intro + {len(slide_segs)} slides + {len(outro_segs)} outro)"
    )
